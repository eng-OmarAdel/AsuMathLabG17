...
{ 
	...
	short FOBPos = input.find('['), SOBPos = input.find('[', FOBPos + 1), FCBPos = input.find(']'), LCBPos = input.rfind(']'), EQPos = input.find('='), plusPos = input.find('+') 
		...
		, transPos = input.find("'"), powPos = input.find("^"); 
	...
	bool mathOP = (plusPos != -1) || (minusPos != -1) || (divPos != -1) || (multPos != -1) || (transPos != -1)|| (powPos != -1); 
...
bool calcAndRep(int i, int j, string  &fullOp, char ch_op) { 
	...
	{ 
		...
		case '/': 
			...
			{ 
				if (memory.p[memoryCheck(s_op1)].getColumns() != memory.p[memoryCheck(s_op2)].getRows() 
					|| memory.p[memoryCheck(s_op2)].getColumns() != memory.p[memoryCheck(s_op2)].getRows()) 
					return 0; 
				string nMat = genRandom(); 
				memory.create(nMat); 
...
{ 
	...
	{ 
		...
		string sFile="", temp;		int Ncomplete=0;		 
		while (getline(infile, temp)) 
		{ 
			if (temp.find("\r") != -1)			temp.replace(temp.find("\r"), 2, "");						sFile+=temp;			if(sFile.find('[')!=-1||sFile.find(']')!=-1){				for(int i=0; i<sFile.length() ;i++)				{					if(sFile[i]=='[')						Ncomplete++;					else if(sFile[i]==']')						Ncomplete--;				}			}							if(Ncomplete==0){			input_checker(sFile); 
				sFile = "";				Ncomplete=0;				}			 
