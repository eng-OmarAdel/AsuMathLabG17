...
void memorizeMatrix(int mIndex, int rows, int columns, string mName)
{
	if (mIndex == -1)
	{
		// cout<<"mwmory"<<memoryPointer;
...
void input_checker(string input) // assignment or operation
{
	...
	{
		removeSpaces(input);
		string tempTrig = input.substr(0, 3);
		...
		else if (tempTrig == "eye")
		{
			...
			string s1(1, input[4]);
			string s3(1, input[6]);
			memory.p[memoryCheck(mName)].eye(StringToDouble(s1), StringToDouble(s1));
		}
		...
		else if (tempTrig == "one")
		{
			...
			string s1(1, input[5]);
			string s3(1, input[7]);
			memory.p[memoryCheck(mName)].ones(StringToDouble(s1), StringToDouble(s1));
		}
		else if (input == "exit")
		{
			exit1 = 1;
